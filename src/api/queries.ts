...
// ── Session report & structural reading aids ────────────────────────────────
...
// The session list and the report itself are queried from `@/hooks/useSessionData`
// and `@/hooks/useSessionReport`: the DECISION A gate has to be one query in one
// place, and a second hook over the same key here would invite a second answer.
